-- Restore ranked result core function dependencies when production has the
-- secure ranked wrappers but is missing the renamed core functions.
-- This is intentionally targeted and safe to apply manually without replaying
...
as $$
  ...
  from jsonb_array_elements(
    case
      when jsonb_typeof(p_move_history) = 'array' then p_move_history
      else '[]'::jsonb
    end
  ) as entry
...
create or replace function public.submit_puzzle_result_core(
  p_session_id uuid,
  p_final_board jsonb,
...
begin
  ...
  v_leaderboard_eligible := v_session.mode in ('classic', 'daily', 'duel') and p_hints_used = 0;
  v_ranked_eligible := v_session.mode in ('ranked', 'ranked_duel');
  ...
  set total_mastery_xp = total_mastery_xp + v_base_xp,
      ...
      duels_played = duels_played + case when v_session.mode in ('duel', 'daily_duel', 'friend_challenge', 'ranked_duel') then 1 else 0 end,
...
create or replace function public.submit_failed_puzzle_result_core(
  p_session_id uuid,
  p_elapsed_seconds integer,
  p_mistakes integer,
  p_hints_used integer,
...
begin
  ...
  if v_session.mode not in ('daily', 'daily_duel', 'friend_challenge', 'ranked', 'ranked_duel') then
    raise exception 'Failed official finalisation is only supported for Daily, Daily Duel, Friend Challenge, Ranked, and Ranked Duel attempts' using errcode = '22023';
  end if;
  ...
  values (
    ...
    0,
    false,
    v_session.mode in ('ranked', 'ranked_duel'),
    v_completed_at
  )
  on conflict do nothing
  returning * into v_result;

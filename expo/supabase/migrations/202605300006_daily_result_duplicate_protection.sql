-- Prevent duplicate completed Daily Sudoku results for the same user and assigned puzzle.

with ranked_daily_results as (
  select
    result_id,
    row_number() over (
      partition by user_id, mode, puzzle_id
...
where gr.result_id = ranked.result_id
  and ranked.rn > 1;

create unique index if not exists idx_game_results_unique_completed_daily
  on public.game_results(user_id, mode, puzzle_id)
  where mode = 'daily'
    and completed = true;
...
begin
  ...
  v_leaderboard_eligible := v_session.mode in ('daily','duel') and p_hints_used = 0;
